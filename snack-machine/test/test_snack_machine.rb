...
class SnackMachineTest < Minitest::Test
  ...
  def test_insert_money_increments_counts
    @snack_machine.insert_money(1, 2, 3, 1, 0, 1)

    assert_equal 1, @snack_machine.one_cent_count
    assert_equal 2, @snack_machine.ten_cent_count
    assert_equal 3, @snack_machine.quarter_count
  ...
  def test_insert_money_multiple_times
    ...
    assert_equal 3, @snack_machine.one_cent_count
    assert_equal 3, @snack_machine.ten_cent_count
    assert_equal 3, @snack_machine.quarter_count
    assert_equal 3, @snack_machine.one_dollar_count
    assert_equal 3, @snack_machine.five_dollar_count
    assert_equal 3, @snack_machine.twenty_dollar_count
  end
end
